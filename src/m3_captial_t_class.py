...
class CapitalT(object):
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        Each rectangle is an rg.Rectangle. Unlike prior modules you are NOT
            ...
            exactly these two and must figure out how to do the
            problem with ONLY
    ...
    def clone(self):
        ...
        # -- t1.v_rect would have an upper left corner of(290, 40)
        # -- t1.v_rect would have an lower right corner of(310, 240)
        pointx = self.h_rect.corner_1.x + abs(self.h_rect.corner_2.x-self.h_rect.corner_1.x)/2
        thickness = abs(self.h_rect.corner_2.y-self.h_rect.corner_1.y)
        intersect = rg.Point(pointx, self.h_rect.corner_1.y + thickness/2)
        width = abs(self.h_rect.corner_2.x - self.h_rect.corner_1.x)
        height = abs(self.v_rect.corner_2.y - self.v_rect.corner_1.y)
        newcap = CapitalT(intersect, width, height, thickness)
        newcap.h_rect.fill_color = self.h_rect.fill_color
        newcap.v_rect.fill_color = self.v_rect.fill_color
        newcap.h_rect.outline_color = self.h_rect.outline_color
        newcap.v_rect.outline_color = self.v_rect.outline_color
        return newcap
